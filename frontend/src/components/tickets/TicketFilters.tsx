...
function FilterChip({ label, onRemove }: { label: string; onRemove: () => void }) {
  return (
    <span className="inline-flex items-center gap-1 rounded-md bg-brand-50 px-2.5 py-1 text-xs font-medium text-brand-700 dark:bg-brand-950/40 dark:text-brand-300">
      {label}
      <button onClick={onRemove} className="rounded hover:bg-brand-100 dark:hover:bg-brand-900/50">
        <X size={12} />
      </button>
    </span>
  );
